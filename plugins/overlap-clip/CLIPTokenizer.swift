...
class CLIPTokenizer {
    ...
    // Overlap: resources are loaded from the plugin directory (a bare executable,
    // not a bundle), so the caller passes the directory instead of Bundle.main.
    init(resourceDir: URL) {

        let url = resourceDir.appendingPathComponent("clip-merges.txt")
        ...
        self.encoder = {
            let url = resourceDir.appendingPathComponent("clip-vocab.json")
    ...
    func encode_full(text: String) -> [Int] {
        // Overlap: CLIP's context is 77 incl. start/end — truncate long queries
